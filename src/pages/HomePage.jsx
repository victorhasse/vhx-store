...
const featured = [
  { id: 1, name: 'Oversized Tee VHX',  category: 'Camisetas',   price: 149, badge: 'Novo' },
  { id: 2, name: 'Cargo Pant Wide',     category: 'Calças',      price: 289, badge: null  },
  { id: 3, name: 'Cap VHX Logo',        category: 'Acessórios',  price: 89,  badge: 'Drop' },
  { id: 4, name: 'Hoodie Acid Wash',    category: 'Moletons',    price: 319, badge: 'Novo' },
]

function ProductCard({ product }) {
  const { id, name, category, price, badge } = product
  return (
    <Link to={`/produtos/${id}`} className="group block bg-[#111] rounded-sm overflow-hidden">
      <div className="relative aspect-3/4 bg-[#1a1a1a] flex items-center justify-center">
      ...
      <div className="p-4">
        <p className="text-[10px] tracking-widest uppercase text-white/40 mb-1">{category}</p>
        <h3 className="text-sm font-medium text-white/90 mb-3 group-hover:text-[#C8F135] transition-colors">{name}</h3>
        <span className="font-display text-xl tracking-wider text-[#C8F135]" style={{fontFamily:'"Bebas Neue",sans-serif'}}>
  ...
  return (
    <div className="bg-[#0a0a0a] min-h-screen">
      ...
      <section className="max-w-7xl mx-auto px-6 pt-16 pb-24 grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
        ...
        {/* Hero visual */}
        <div className="relative hidden md:flex items-center justify-center h-[480px] bg-[#111] rounded-sm overflow-hidden">
          <p style={{fontFamily:'"Bebas Neue",sans-serif'}} className="text-[180px] text-white/5 tracking-tight select-none">VHX</p>
          <div className="absolute inset-0 flex items-center justify-center">
            <svg xmlns="http://www.w3.org/2000/svg" className="w-32 h-32 text-[#C8F135]/20" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={0.5}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M20.25 7.5l-.625 10.632a2.25 2.25 0 01-2.247 2.118H6.622a2.25 2.25 0 01-2.247-2.118L3.75 7.5M10 11.25h4M3.375 7.5h17.25c.621 0 1.125-.504 1.125-1.125v-1.5c0-.621-.504-1.125-1.125-1.125H3.375c-.621 0-1.125.504-1.125 1.125v1.5c0 .621.504 1.125 1.125 1.125z" />
            </svg>
          </div>
          <span className="absolute bottom-4 right-4 text-[10px] tracking-widest uppercase text-white/20">Hero Image</span>
        </div>
      </section>
        ...
        <div className="grid grid-cols-3 gap-3">
          {[
            ...
            <Link
              key={label}
              to="/produtos"
              className="group relative h-32 bg-[#111] rounded-sm overflow-hidden flex flex-col justify-end p-4 hover:bg-[#1a1a1a] transition-colors"
            >
