  ...
  return (
    <div className="flex gap-6 py-6 border-b border-white/5">
      ...
      <div className="w-24 h-24 bg-[#111] rounded-sm flex items-center justify-center flex-shrink-0">
        ...
        <svg xmlns="http://www.w3.org/2000/svg" className="w-8 h-8 text-white/10" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1}>
      ...
      <div className="flex-1 flex flex-col justify-between">
        <div className="flex justify-between items-start">
          <div>
            <p className="text-[10px] tracking-widest uppercase text-white/30 mb-1">{CATEGORY_LABELS[item.category] || item.category}</p>
  ...
  if (items.length === 0) {
    return (
      <div className="bg-[#0a0a0a] min-h-screen flex flex-col items-center justify-center gap-6">
        <p style={{fontFamily:'"Bebas Neue",sans-serif'}} className="text-6xl tracking-widest text-white/10">
          {t('cart.empty')}
        </p>
          ...
          <div className="lg:col-span-1">
            <div className="bg-[#111] rounded-sm p-6 sticky top-24">
              ...
              <div className="space-y-3 mb-6">
                ...
                <div className="flex justify-between text-sm">
                  <span className="text-white/40 tracking-wider">{t('cart.shipping')}</span>
                  <span className="text-[#C8F135] text-xs tracking-widest uppercase">{t('cart.free')}</span>
                </div>
              </div>

              <div className="border-t border-white/5 pt-4 mb-8">
                <div className="flex justify-between items-baseline">
                  <span className="text-sm text-white/40 tracking-wider">{t('cart.total')}</span>
